...
console.log(`Bütün komutlar başarıyla yüklendi!`);
  ...
  client.user.setActivity('!yardım | !eğlence | !moderasyon | !eğlence2');
})
...
client.on('message', async message => {
  if (message.content.startsWith('!çal')) {
    ...
    if (!botmesajı) return message.reply('URL yazmayaı unuttunuz')
    if (message.member.voice.channel) {
      const connection = await message.member.voice.channel.join();
      const ytdl = require('ytdl-core');
      connection.play(ytdl(`${botmesajı}`, { filter: 'audioonly'}))
    } else {
      message.reply('**Bir Sesli kanala katılman gerekir**')
    }
  }
})

client.on("guildMemberAdd", member => {
  let role = member.guild.roles.cache.find(role => role.id === `753965058349858946`);
  member.roles.add(role);
})

client.on('guildMemberAdd', member => {
  try {
  const Sayaç = member.guild.channels.catch.find(channel => channel.id === '753982052998840401');
...
client.on('ready', () => {
  client.channels.cache.get('819882158856208384').join()//ANA SUNUCU
  
});

client.on('ready', () => {
  client.channels.cache.get('821780005138858054').join()//SUPPORT SUNUCU
